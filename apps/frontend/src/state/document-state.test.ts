...
describe('Document State Model', () => {
  ...
  describe('Initial State', () => {
    ...
    it('should initialize with null hovered ID', () => {
      expect(state.hoveredId.get()).toBeNull();
    });
  ...
  describe('Selection Updates', () => {
    ...
    it('should replace previous selection when selecting', () => {
      updater.select(['rect1']);
      updater.select(['circle1']);

      const ids = state.selectedIds.get();
      expect(ids.size).toBe(1);
      expect(ids.has('rect1')).toBe(false);
      expect(ids.has('circle1')).toBe(true);
    });

    it('should add to existing selection', () => {
      updater.select(['rect1']);
      updater.addToSelection(['circle1', 'ellipse1']);

      const ids = state.selectedIds.get();
      expect(ids.size).toBe(3);
      expect(ids.has('rect1')).toBe(true);
      expect(ids.has('circle1')).toBe(true);
      expect(ids.has('ellipse1')).toBe(true);
    });

    it('should remove from selection', () => {
      updater.select(['rect1', 'circle1', 'ellipse1']);
      updater.removeFromSelection(['circle1']);

      const ids = state.selectedIds.get();
      expect(ids.size).toBe(2);
      expect(ids.has('rect1')).toBe(true);
      expect(ids.has('circle1')).toBe(false);
      expect(ids.has('ellipse1')).toBe(true);
    });
    ...
    it('should set hovered ID', () => {
      updater.setHoveredId('rect1');
      expect(state.hoveredId.get()).toBe('rect1');

      updater.setHoveredId(null);
      expect(state.hoveredId.get()).toBeNull();
    });
  });
  ...
  describe('Computed Values', () => {
    ...
    it('should compute selectionCount correctly', () => {
      ...
      updater.addToSelection(['circle1', 'ellipse1']);
      expect(state.selectionCount.get()).toBe(3);

      updater.removeFromSelection(['circle1']);
      expect(state.selectionCount.get()).toBe(2);
    });
    ...
    it('should compute selectedElements from document', () => {
      ...
      const mockTree: DocumentNode[] = [];
      updater.setDocument(svg, mockTree, '');
    ...
    it('should filter out non-existent IDs from selectedElements', () => {
      const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
      const rect = document.createElementNS('http://www.w3.org/2000/svg', 'rect');
      rect.setAttribute('id', 'rect1');
      svg.appendChild(rect);
      ...
      updater.select(['rect1', 'nonexistent']);
    ...
    it('should find nested nodes in selectedNodes', () => {
      const mockTree: DocumentNode[] = [
        {
          id: 'g1',
          type: 'element',
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle duplicate IDs in selection', () => {
      updater.select(['rect1', 'rect1', 'rect1']);
      expect(state.selectedIds.get().size).toBe(1);
    });

    it('should handle adding duplicate IDs to selection', () => {
      updater.select(['rect1']);
      updater.addToSelection(['rect1', 'circle1']);
      expect(state.selectedIds.get().size).toBe(2);
    });
    ...
    it('should handle removing non-existent IDs', () => {
      ...
      expect(state.selectedIds.get().size).toBe(1);
      expect(state.selectedIds.get().has('rect1')).toBe(true);
    });

    it('should handle null document in selectedElements', () => {
      updater.select(['rect1']);
      expect(state.selectedElements.get()).toEqual([]);
    });

    it('should handle empty tree in selectedNodes', () => {
      updater.select(['rect1']);
      expect(state.selectedNodes.get()).toEqual([]);
    });
  });
